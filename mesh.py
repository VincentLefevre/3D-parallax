...
def output_3d_photo(verts, colors, faces, Height, Width, hFov, vFov, ref_pose, int_mtx, config, pt_view, img, cam_mesh, original_H=None, original_W=None,
                    border=None, normal_canvas=None, all_canvas=None, mean_loc_depth=None):
    ...
    rel_pose = np.linalg.inv(np.dot(pt_view[0], np.linalg.inv(ref_pose)))
    ...
    crop_stereo = (stereo[atop:abuttom, aleft:aright, :3] * 1).astype(np.uint8)
    stereo = crop_stereo

    Img_3D = np.array(stereo)
