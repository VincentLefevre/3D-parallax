...
def path_planning(num_frames, x, y, z, path_type=''):
    if path_type == 'straight-line':
        ...
        spline = cs(t) #[x,y,z,x1,y1,z1]  [[x,y,z],[x1,y1,z1]]
        ...
        xs, ys, zs = [], [], []
        #tracé des longueurs selon x
        corner_points = np.array([[0, 0, 0], [(0 + x) * 0.5, (0 + 0) * 0.5, (0 + z) * 0.5], [x, 0, z]])
        corner_t = np.linspace(0, 1, len(corner_points))
        t = np.linspace(0, 1, floor(num_frames/4))
        cs = interp1d(corner_t, corner_points, axis=0, kind='quadratic')
        spline = cs(t)
        xs, ys, zs = [xx.squeeze() for xx in np.split(spline, 3, 1)]
        #tracé des longueurs selon y
        corner_points = np.array([[x, 0, z], [(x + x) * 0.5, (0 + y) * 0.5, (z + z) * 0.5], [x, y, z]])
        corner_t = np.linspace(0, 1, len(corner_points))
        t = np.linspace(0, 1, floor(3*num_frames/4))
        cs = interp1d(corner_t, corner_points, axis=0, kind='quadratic')
        spline = cs(t) 
        Lxs, Lys, Lzs = [xx.squeeze() for xx in np.split(spline, 3, 1)]
        xs = np.append(xs, Lxs)
        ys = np.append(ys, Lys)
        zs = np.append(zs, Lzs)
    elif path_type == 'uncontinuous-lines':
        corner_points = np.array([[x, 0, z], [(0 + x) * 0.5, (0 + 0) * 0.5, (0 + z) * 0.5], [0,0,0]])
        corner_t = np.linspace(0, 1, len(corner_points))
        t = np.linspace(0, 1, floor(num_frames/2))
        cs = interp1d(corner_t, corner_points, axis=0, kind='quadratic')
        spline = cs(t) 
        xs, ys, zs = [xx.squeeze() for xx in np.split(spline, 3, 1)]
        xs = np.append(xs, xs)
        ...
        r = x/y
        L = np.linspace(-y,y,floor(sqrt(num_frames/r)))
        xs, ys, zs = [], [], []
        for y0 in L : # Balayage de droite à gauche et de bas en haut
            corner_points = np.array([[-x, y0, 0], [0, y0,0], [x, y0, 0]])
            corner_t = np.linspace(0, 1, len(corner_points))
            t = np.linspace(0, 1,floor(sqrt(num_frames*r)))
            cs = interp1d(corner_t, corner_points, axis=0, kind='quadratic')
            spline = cs(t)
            Lxs, Lys, Lzs = [xx.squeeze() for xx in np.split(spline, 3, 1)]
            xs = np.append(xs, Lxs)
...
def get_MiDaS_samples(image_folder, depth_folder, config, specific=None, aft_certain=None):
    ...
    for traj_idx in range(len(config['traj_types'])):
        ...
        tgts_poses += [tgt_poses] #liste contenant les chemins pour chaque type de trajectoire
    tgt_pose = generic_pose * 1
